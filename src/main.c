...
#ifndef WRKX_CONFIGURE_FLAGS
#define WRKX_CONFIGURE_FLAGS ""
#endif
#ifndef WRKX_EXTENSIONS
#define WRKX_EXTENSIONS ""
#endif
...
static void print_config_section(const cli_args *a) {
    const char *flags = WRKX_CONFIGURE_FLAGS;
    const char *exts  = WRKX_EXTENSIONS;

    printf("Configuration\n");
    print_rule();
    printf("wrkx %s [%s]\n", CLI_VERSION, aeGetApiName());
    printf("Build:       ./configure %s\n", flags);
    if (exts[0])
        printf("Built-in:    %s\n", exts);
    else
        printf("Built-in:    (none)\n");
    printf("\n");
    printf("URL:         %s\n", a->url);
    printf("Threads:     %" PRIu64 "\n", a->cfg.threads);
    printf("Connections: %" PRIu64 "\n", a->cfg.connections);
    char *dur = format_time_s(a->cfg.duration_us / 1000000ULL);
    printf("Duration:    %s\n", dur);
    free(dur);
    if (a->cfg.rate > 0)
        printf("Rate:        %" PRIu64 "/s\n", a->cfg.rate);
    if (a->script)
        printf("Script:      %s\n", a->script);
}
