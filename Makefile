...
# Build-info macros surfaced in the Configuration section of wrkx output.
# WRKX_CONFIGURE_FLAGS is the canonical capability string shown in the Build:
# line — always includes --with-lua, adds --with-quickjs when enabled, and
# appends any non-feature configure flags (--with-openssl=PATH, --prefix=PATH).
WRKX_CONFIGURE_FLAGS := --with-lua
ifeq ($(QUICKJS_ENABLED),1)
WRKX_CONFIGURE_FLAGS += --with-quickjs
endif
ifneq ($(CONFIGURE_FLAGS),)
WRKX_CONFIGURE_FLAGS += $(CONFIGURE_FLAGS)
endif
CFLAGS += -DWRKX_CONFIGURE_FLAGS='"$(WRKX_CONFIGURE_FLAGS)"'
CFLAGS += -DWRKX_EXTENSIONS='"$(EXTENSIONS)"'
